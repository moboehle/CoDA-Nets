<div style="margin-left:auto; margin-right:auto; padding:0; text-align:center">
  <img width="160px" height="auto" style="vertical-align:middle; display:inline; margin:0; padding:0" src="media/lorikeet.gif?raw=true"/>
  <img width="160px" height="auto" style="vertical-align:middle; display:inline; margin:0; padding:0" src="media/drake2_atts.gif?raw=true"/>
  <img width="160px" height="auto" style="vertical-align:middle; display:inline; margin:0; padding:0" src="media/birds_atts.gif?raw=true"/>
  <img width="160px" height="auto" style="vertical-align:middle; display:inline; margin:0; padding:0" src="media/goldfinch.gif?raw=true"/>
</div>
# Introduction
...
computed matrices, 
the entire CoDA network also performs a linear transformation of the input: the intermediate matrices can be collapsed
to a single linear transformation. Importantly, the output of the network
...
has to align the dynamic weights with the input. As we discuss in our [paper](https://arxiv.org/pdf/2104.00032.pdf),
we can take advantage of this and artificially lower the network outputs by dividing it by a 'temperature' parameter T.
As a result, in order to achieve the same output magnitude, the network needs to achieve better alignment throughout the network
the bigger the temperature parameter is. 
<div style="displaystyle=block; align=center; margin: auto">
  <img class="zoom2" src="media/alignment_pressure.jpeg"/>
</div>
And indeed, as can be seen qualitatively in the above figure, this is indeed what we observe for models trained with different
temperatures.
