...
def get_model(exp_params):
    ...
    network = make_coda_net(emb, out_c, bn_ch, ks, stride, padding, norm, logit_bias, final_norm)

    # For the regularisation experiments, need to save the intermediate computations.
    if exp_params["regul_matrix"]:

        for mod in network.modules():
            if hasattr(mod, "save_matrix"):
                mod.save_matrix = True
